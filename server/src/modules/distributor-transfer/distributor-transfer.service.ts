...
import { TransferGrid, TransferSummaryBuilder } from 'src/types/distributor-transfer.types.js';
...
export class DistributorTransferService {
    ...
    async generateTransfer(paperId: number) {
        ...
        const sourceItems =
            await this.repository.getTransferSourceItems(paper.id);
        ...
        this.validation.validateTransferRules(
            summaries,
            transferRules,
        );

        const transfers = this.buildTransferEntities(
            paper.id,
            summaries,
        );
