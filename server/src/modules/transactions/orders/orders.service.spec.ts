...
import { OrderPaperStatus, SupplyCategory } from '../../../generated/prisma/client.js';
import { ERROR_MESSAGES, SUCCESS_MESSAGES } from './orders.constants.js';
import { DEPENDENCY_MODULES, DEPENDENCY_TRIGGERS } from '../dependencies/dependency.constant.js';
...
describe('OrdersService', () => {
    ...
    describe('getAvailableProducts', () => {
        it('delegates category to repository', async () => {
            ...
            expect(
                ordersRepository.findAvailableProducts,
            ).toHaveBeenCalledWith(SupplyCategory.MILK);
    ...
    describe('getSheetService', () => {
        ...
        it('loads all required sheet data and builds billing section', async () => {
            ...
            expect(ordersRepository.getSheetItems).toHaveBeenCalledWith(
                10,
                prisma,
            );

            expect(
                workflowBuilder.buildOrdersWorkflow,
            ).toHaveBeenCalledWith(OrderPaperStatus.DRAFT);

            expect(
                ordersBuilder.buildOrderBillingSection,
    ...
    describe('getSheetItemsService', () => {
        it('rejects an invalid sheet id', async () => {
            await expect(
                service.getSheetItemsService(0),
            ).rejects.toBeInstanceOf(BadRequestException);
        ...
        it('rethrows repository errors', async () => {
            ...
            await expect(
                service.getSheetItemsService(10),
            ).rejects.toBe(error);
        });
    });
    ...
    describe('addProductToSheet', () => {
        ...
        it('validates and resolves the commercial context before creating the sheet product', async () => {
            ...
            vi.spyOn(service, 'getSheetService').mockResolvedValue(
                {} as any,
            );

            await service.addProductToSheet(10, dto as any);

            expect(
                validationService.validateProduct,
            ).toHaveBeenCalledWith(50, prisma);
    ...
    describe('removeProductFromSheet', () => {
        ...
        it('rejects a default product', async () => {
            ...
            await expect(
                service.removeProductFromSheet(10, 50),
            ).rejects.toThrow('Default products cannot be removed');
        });
        ...
        it('rejects when the product is not in the sheet', async () => {
            ...
            await expect(
                service.removeProductFromSheet(10, 50),
            ).rejects.toThrow('Product not found in this sheet');
        });
        ...
        it('deletes the product and its sheet items', async () => {
            ...
            expect(
                ordersRepository.deleteSheetProduct,
            ).toHaveBeenCalledWith(10, 50, prisma);

            expect(
                ordersRepository.deleteSheetItems,
            ).toHaveBeenCalledWith(10, 50, prisma);
        });
    });
...
it('rejects an entry with an undefined orderedQty before saving anything', async () => {
    const entriesWithUndefinedQty = [
        { clientId: 1, productId: 2 } as any,
    ];
    ...
    expect(billingService.saveNightEntriesBatch).not.toHaveBeenCalled();
        ...
        it('validates entries and quantities before saving', async () => {
            await service.saveNightEntriesService(10, entries as any);

            expect(
                validationService.validateEntriesBatch,
            ).toHaveBeenCalledWith(
                entries,
                20,
                prisma,
            ...
            expect(
                billingService.getTrayRulesOnce,
            ...
            expect(
                billingService.saveNightEntriesBatch,
            ).toHaveBeenCalledWith(
                prisma,
                sheet,
                expect.anything(),
        ...
        it('executes Orders to Client Trays propagation after saving', async () => {
            ...
            expect(
                dependencyOrchestrator.execute,
            ).toHaveBeenCalledWith(
        ...
        it('returns the success response', async () => {
            const result =
                await service.saveNightEntriesService(
                    10,
                    entries as any,
                );
    ...
    describe('saveMorningEntriesService', () => {
        ...
        it('validates quantities and saves morning entries', async () => {
            await service.saveMorningEntriesService(10, entries as any);
            ...
            expect(
                billingService.saveMorningEntriesBatch,
            ).toHaveBeenCalledWith(
                prisma,
                sheet,
                10,
            ...
            expect(
                ordersRepository.markOrderMorningEntrySaved,
            ).toHaveBeenCalledWith(
                10,
                prisma,
            );
        ...
        it('returns the success response', async () => {
            const result =
                await service.saveMorningEntriesService(
                    10,
                    entries as any,
                );
