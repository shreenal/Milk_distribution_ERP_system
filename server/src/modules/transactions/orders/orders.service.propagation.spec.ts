...
describe('OrdersService propagation (integration)', () => {
    ...
    beforeEach(async () => {
        ...
        clientTraysPropagationService =
            new ClientTraysPropagationService(
    ...
    async function createPaperAndSheet() {
        ...
        const sheets = await paperRepository.generateOrderSheets(
            paper.id,
            [group!],
        );
    ...
    it('propagates night save to Client Trays using ordered_qty', async () => {
        ...
        const { client, product } =
            await getTestClientAndProduct(sheet.id);
        ...
        const trayTransactions =
            await testPrisma.client_tray_transaction.findMany({
                where: {
                    order_sheet_id: sheet.id,
                    client_id: client.id,
                },
        ...
        const totalTraysTaken = trayTransactions.reduce(
            (sum, transaction) =>
                sum + Number(transaction.trays_taken ?? 0),
            0,
        );
    ...
    const item = await testPrisma.order_sheet_items.findFirstOrThrow({
        where: { order_sheet_id: sheet.id, client_id: client.id, product_id: shaTonedProduct.id },
...
it('re-resolves only the genuinely new product when a batch mixes a new and an already-pinned product', async () => {
    ...
    expect(pinAfter.created_at.toISOString()).toBe(pinBefore.created_at.toISOString());
    ...
    const itemOne = await testPrisma.order_sheet_items.findFirstOrThrow({
        where: { order_sheet_id: sheet.id, client_id: client.id, product_id: productOne.id },
    });
    ...
    it('preserves manually entered trays_returned during Orders propagation', async () => {
        ...
        const transaction =
            await testPrisma.client_tray_transaction.findFirst({
                where: {
                    order_sheet_id: sheet.id,
                    client_id: client.id,
                },
        ...
        const after =
            await testPrisma.client_tray_transaction.findUnique({
                where: {
                    id: transaction!.id,
                },
            });
    ...
    it('propagates from a REOPENED sheet to later sheets in the same delivery group', async () => {
        ...
        const { client, product } = await getTestClientAndProduct(
            firstSheet!.id,
        );
        ...
        const beforeOpeningBalance = Number(
            before!.opening_balance,
        );

        const beforeTraysTaken = Number(
            before!.trays_taken ?? 0,
        );

        const beforeTraysReturned = Number(
            before!.trays_returned ?? 0,
        );
         ...
         */
        expect(Number(after!.trays_taken ?? 0)).toBe(
            beforeTraysTaken,
        );

        expect(Number(after!.trays_returned ?? 0)).toBe(
            beforeTraysReturned,
        );
         ...
         */
        expect(Number(after!.opening_balance)).not.toBe(
