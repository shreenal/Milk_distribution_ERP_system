...
import { Prisma, OrderPaperStatus, SupplyCategory } from '../../../generated/prisma/client.js';
import {
...
describe('OrdersRepository (integration)', () => {
    ...
    describe('client queries', () => {
        it('getClientsByGroupId returns active clients ordered by name', async () => {
            ...
            for (let i = 1; i < result.length; i++) {
                expect(result[i - 1].name.localeCompare(result[i].name)).toBeLessThanOrEqual(
                    0,
                );
            }
        });
    ...
    describe('sheet product links', () => {
        ...
        it('getSheetProductLink returns the pinned link', async () => {
            ...
            const result = await repository.getSheetProductLink(
                sheet.id,
                product.id,
            );
        ...
        it('deleteSheetProduct removes the sheet product link', async () => {
            ...
            const result = await repository.deleteSheetProduct(
                sheet.id,
                product.id,
            );
    ...
    describe('sheet item writes', () => {
        ...
        it('deleteSheetItems deletes all items for the sheet and product', async () => {
            ...
            const result = await repository.deleteSheetItems(
                sheet.id,
                product.id,
            );
    ...
    describe('selling rates', () => {
        ...
        it('getSellingRate prefers the applicable client rate', async () => {
            ...
            const distributorRate =
                await testPrisma.distributor_product_rate.create({
                    data: {
                        product_link_id: link.id,
                        purchase_rate: 80,
                        selling_rate: 100,
            ...
            const clientRate =
                await testPrisma.master_client_rate_product.create({
                    data: {
                        client_id: client.id,
                        product_link_id: link.id,
                        selling_rate: 125,
        ...
        it('getSellingRatesBatch prefers client rates and falls back to distributor rates', async () => {
            ...
            const clientRateLink =
                await getProductLink(productWithClientRate.id);
            ...
            expect(
                result.get(`${client.id}_${clientRateLink.id}`),
            ).toEqual(new Prisma.Decimal(25));

            expect(
                result.get(`${client.id}_7`),
            ).toEqual(new Prisma.Decimal(28.20));
        });
    });
    ...
    describe('getEligibleDistributorsForProduct', () => {
        it('returns eligible distributors ordered by priority with the primary distributor at priority zero', async () => {
            ...
            const result =
                await repository.getEligibleDistributorsForProduct(
                    group.id,
                    product.id,
                    product.brand_id,
            ...
            expect(result.length).toBeGreaterThan(0);
            expect(result.some((item) => item.distributorId === primaryDistributorId)).toBe(
                true,
            );
            ...
            for (let i = 1; i < result.length; i++) {
                expect(result[i - 1].priority).toBeLessThanOrEqual(
                    result[i].priority,
                );
            }
        });
    ...
    it('persists resolved_via_fallback=true when the primary distributor has no link and a lower-priority distributor is used', async () => {
        ...
        const expectedLink = await testPrisma.master_product_link.findFirstOrThrow({
            where: {
