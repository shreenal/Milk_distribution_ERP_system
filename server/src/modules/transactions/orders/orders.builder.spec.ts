...
describe('OrdersBuilder', () => {
  ...
  const nonMilkClients = [{ id: 201, name: 'Client C' }];
  ...
  function mockColumns() {
    productColumnsBuilder.buildGroupedColumns.mockImplementation(
      (products: any[], isNonMilk: boolean) => ({
        products,
        isNonMilk,
      }),
    );
  ...
  describe('product columns', () => {
    it('builds milk and non-milk columns with the correct category flag', () => {
      ...
      expect(productColumnsBuilder.buildGroupedColumns).toHaveBeenCalledTimes(
        2,
      );

      expect(productColumnsBuilder.buildGroupedColumns).toHaveBeenNthCalledWith(
        1,
        milkProducts,
        false,
      );
  ...
  describe('ordered quantity mode', () => {
    it('uses ordered_qty and night billing in DRAFT', () => {
      ...
      expect(workflowState.resolveUseOrderedQuantity).toHaveBeenCalledWith(
        OrderPaperStatus.DRAFT,
        false,
      );
