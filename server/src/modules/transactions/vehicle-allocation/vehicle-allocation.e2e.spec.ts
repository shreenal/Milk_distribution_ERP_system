...
describe('VehicleAllocationController (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    orderDate.setUTCDate(orderDate.getUTCDate() + 2000 + fixtureCounter++);
  ...
  describe('GET /vehicle-allocations/:paperId/vehicle-allocations', () => {
    it('returns vehicle allocation data for an authenticated employee', async () => {
      ...
      expect(response.body.vehicleAssignments).toBeDefined();
      expect(response.body.requirementGrids).toBeDefined();
      expect(response.body.vehicleAllocationPaperUpdatedAt).toBeNull();
    });

    it('rejects an invalid paperId', async () => {
      const response = await request(server())
        .get('/vehicle-allocations/not-a-number/vehicle-allocations')
        .set('Authorization', `Bearer ${accessToken}`)
        .query({
          session: 'MORNING',
        });
