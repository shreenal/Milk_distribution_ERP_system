...
describe('VehicleAllocationService concurrency (integration, plan §13)', () => {
    ...
    it('exactly one of two concurrent saves with the same expectedUpdatedAt succeeds; the other receives 409', async () => {
        ...
        // baseline updated_at.
        await vehicleAllocationService.saveVehicleAllocations(paper.id, baseDto as any);
        ...
        const finalAllocation = await testPrisma.vehicle_allocation.findFirstOrThrow({
            where: {
