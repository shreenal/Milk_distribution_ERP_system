...
function createService() {
    const repository = new VehicleAllocationRepository(
    ...
    describe('saveVehicleAllocations', () => {
        it('persists assignments and allocations through the real transaction', async () => {
            ...
            const allocationPaper =
                await prisma.vehicle_allocation_paper.findUnique({
                    where: {
        ...
        it('rejects a stale expectedUpdatedAt without changing database state', async () => {
            ...
            const orderPaper = await createOrderPaper();
            const allocationPaper =
                await createAllocationPaper(orderPaper.id);
            ...
            const beforeAllocation =
                await prisma.vehicle_allocation.findUnique({
            ...
            await expect(
                service.saveVehicleAllocations(
                    orderPaper.id,
                    dto as any,
                ),
            ...
            expect(afterAllocation!.id).toBe(
                beforeAllocation!.id,
            );
            ...
            expect(afterAssignment!.id).toBe(
                beforeAssignment!.id,
            );
        ...
        it('does not modify the allocation paper timestamp on an identical save', async () => {
            ...
            await service.saveVehicleAllocations(
                orderPaper.id,
                dto as any,
            );
            ...
            const beforeTimestamp =
                allocationPaper!.updated_at.getTime();
            ...
            const secondResult =
                await service.saveVehicleAllocations(
                    orderPaper.id,
                    {
                        ...dto,
                        expectedUpdatedAt:
                            allocationPaper!.updated_at.toISOString(),
                    } as any,
                );
        ...
        it('updates an allocation in place through the service', async () => {
            ...
            const before = await getAllocations(
                allocationPaper!.id,
            );
            ...
            const after = await getAllocations(
                allocationPaper!.id,
            );
        ...
        it('sets purchase_entry.source_allocation_id to NULL when a saved allocation is removed', async () => {
            ...
            const allocationRows = await getAllocations(
                allocationPaper!.id,
            );
            ...
            const purchasePaper =
                await prisma.purchase_paper.create({
                    data: {
                        order_paper_id: orderPaper.id,
                    },
                });
            ...
            const productLink =
                await prisma.master_product_link.findUnique({
                    where: {
            ...
            if (!productLink) {
                throw new Error(
                    'Expected seeded product link was not found.',
                );
            }

            const purchaseEntry =
                await prisma.purchase_entry.create({
                    data: {
            ...
            const updatedPurchaseEntry =
                await prisma.purchase_entry.findUnique({
                    where: { id: purchaseEntry.id },
                });

            expect(
                updatedPurchaseEntry!.source_allocation_id,
            ).toBeNull();
        });
        ...
        it('rolls back assignments and allocations when persistence fails', async () => {
            ...
            const beforeAllocations =
                await getAllocations(allocationPaper.id);

            const beforeAssignments =
                await getAssignments(allocationPaper.id);
            ...
            expect(afterAllocations[0].id).toBe(
                existingAllocation.id,
            );
            expect(
                Number(afterAllocations[0].allocated_qty),
            ).toBe(10);

            expect(afterAssignments).toHaveLength(1);
            expect(afterAssignments[0].id).toBe(
                existingAssignment.id,
            );
            expect(afterAssignments[0].vehicle_id).toBe(
                vehicle1.id,
            );
            expect(afterAssignments[0].distributor_id).toBe(
                distributorA.id,
            );
            expect(afterAssignments[0].category).toBe(
                SupplyCategory.MILK,
            );
        });
    });
