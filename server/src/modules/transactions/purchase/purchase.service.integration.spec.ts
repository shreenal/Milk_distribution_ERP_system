...
const repository = new PurchaseRepository(prisma);
...
async function createAllocationFixture(params: {
  ...
  const allocationPaper = await createNightAllocationPaper(params.orderPaperId);
...
function createService() {
  ...
  const purchaseValidationService = {
    ...
    validatePurchases: vi.fn().mockImplementation(async (paperId, dto, db) => {
      const allocations = await repository.findVehicleAllocationsByPaperId(
        paperId,
        db,
      );
      ...
      for (const entry of dto.entries) {
        ...
        if (Number(entry.purchasedQty) > Number(allocation.allocated_qty)) {
          throw new Error(
  ...
  const trayCalculationService = {
    ...
    buildTransaction: vi
      .fn()
      .mockImplementation((openingBalance, traysTaken, traysReturned) => ({
        opening_balance: openingBalance,
        trays_taken: traysTaken,
        trays_returned: traysReturned,
        closing_balance: openingBalance + traysTaken - traysReturned,
      })),
  };
...
describe('PurchaseService - PostgreSQL integration', () => {
  ...
  it('does not require a live rate for an allocation that already has a saved purchase entry', async () => {
    const { vehicle, distributor, product, productLink } = await getSeedData();
    ...
    try {
      ...
      const productLinkBatchSpy = vi.spyOn(repository, 'getProductLinksBatch');
      ...
      const [productLinkRequests] = productLinkBatchSpy.mock.calls[0];

      expect(productLinkRequests).toEqual([]);

      expect(rateBatchSpy).toHaveBeenCalledTimes(1);

      const [rateRequests] = rateBatchSpy.mock.calls[0];
      ...
      await prisma.vehicle_distribution_assignment.deleteMany({
        where: {
          vehicle_allocation_paper_id: allocation.vehicle_allocation_paper_id,
        },
      });
  ...
  it('throws when an unsaved allocation requires a live rate that has lapsed', async () => {
    const { vehicle, distributor, product, productLink } = await getSeedData();
    ...
    try {
      ...
      await expect(service.getPurchases(orderPaper.id)).rejects.toThrow(
  ...
  describe('getPurchases - RepeatableRead consistency', () => {
    it('keeps a consistent snapshot when a vehicle allocation is committed during the read', async () => {
      ...
      try {
        ...
        vi.spyOn(repository, 'findOrderPaperById').mockImplementation(
          async (paperId, db) => {
            const result = await originalFindOrderPaperById(paperId, db);
        ...
        await concurrentPrisma.vehicle_allocation.create({
          data: {
            vehicle_allocation_paper_id: concurrentAllocationPaper.id,
        ...
        vi.spyOn(purchaseBuilder, 'applyVehicleAllocations').mockImplementation(
          (grids, allocations) => {
         ...
         * MORNING allocation.
         */
        const allocationPaperSessions = result.purchases.flatMap((purchase) =>
          purchase.rows.map((row) => row.deliverySession),
        );

        expect(allocationPaperSessions).not.toContain(DeliverySession.MORNING);
      } finally {
        vi.restoreAllMocks();
    ...
    it('flags a saved purchase row as stale when source_allocation_id no longer matches', async () => {
      ...
      try {
        ...
        await prisma.vehicle_allocation.deleteMany({
          where: {
            id: {
              in: [allocation.id, previousAllocation.id],
            },
          },
        });
        ...
        await prisma.order_paper.deleteMany({
          where: {
            id: {
              in: [orderPaper.id, previousOrderPaper.id],
            },
          },
        });
  ...
  describe('getPurchases - morningAllocationPending', () => {
    it('returns true when no MORNING allocation paper exists', async () => {
      const { vehicle, distributor, product } = await getSeedData();
    ...
    it('returns false when a MORNING allocation paper exists even when it has zero requirements', async () => {
      const { vehicle, distributor, product } = await getSeedData();
      ...
      try {
        ...
        const morningAllocations = await prisma.vehicle_allocation.findMany({
          where: {
            vehicle_allocation_paper_id: morningAllocationPaper.id,
          },
        });
  ...
  describe('getPurchases - orphanedEntries', () => {
    it('surfaces a saved purchase row missing from the current allocation grid without duplicating it in purchases.rows', async () => {
      const { vehicle, distributor, product, productLink } =
        await getSeedData();
      ...
      try {
        ...
        expect(
          purchaseRows.some((row: any) => row.id === purchaseEntry.id),
        ).toBe(false);
      } finally {
        await prisma.purchase_entry.deleteMany({
          where: {
  ...
  describe('savePurchases - optimistic concurrency', () => {
    it('rejects a stale expectedUpdatedAt without writing purchase entries', async () => {
      const { vehicle, distributor, product } = await getSeedData();
      ...
      try {
        ...
        const staleUpdatedAt = new Date(currentUpdatedAt.getTime() - 60_000);
        ...
        const purchasePaperAfter = await prisma.purchase_paper.findUnique({
          where: {
            id: purchasePaper.id,
          },
        });

        expect(purchasePaperAfter?.updated_at.getTime()).toBe(
  ...
  describe('savePurchases - deletion confirmation', () => {
    it('rejects deletion of existing purchase entries when confirmDeletions is false', async () => {
      const { vehicle, distributor, product, productLink } =
        await getSeedData();
      ...
      try {
        ...
        await expect(
          service.savePurchases(orderPaper.id, {
            entries: [],
            confirmDeletions: false,
          } as any),
        ).rejects.toMatchObject({
        ...
        const entryAfter = await prisma.purchase_entry.findUnique({
          where: {
            id: purchaseEntry.id,
          },
        });
    ...
    it('deletes existing purchase entries when confirmDeletions is true', async () => {
      const { vehicle, distributor, product, productLink } =
        await getSeedData();
      ...
      try {
        ...
        await service.savePurchases(orderPaper.id, {
          entries: [],
          confirmDeletions: true,
        });
  ...
  describe('savePurchases - tray type behavior', () => {
    it('preserves the existing purchase entry tray_type_id when saving again', async () => {
      const { vehicle, distributor, product, productLink } =
        await getSeedData();
      ...
      try {
        ...
        const savedEntry = await prisma.purchase_entry.findUnique({
          where: {
            id: purchaseEntry.id,
          },
        });
        ...
        expect(savedEntry).not.toBeNull();
        expect(savedEntry!.tray_type_id).toBe(frozenTrayTypeId);
  ...
  describe('savePurchases - assignment matching', () => {
    it('rejects when the vehicle is assigned to a different distributor', async () => {
      ...
      try {
        ...
        await expect(service.savePurchases(orderPaper.id, dto)).rejects.toThrow(
          PURCHASE_ERROR_MESSAGES.VEHICLE_ASSIGNMENT_NOT_FOUND(vehicle.id),
        );
  ...
  describe('savePurchases - gatepass date policy', () => {
    it('uses the previous calendar day for PREVIOUS_DAY policy', async () => {
      ...
      const originalPolicy = product.master_brand.gatepass_date_policy;
  ...
  describe('savePurchases - new entry tray type', () => {
    it('uses the current tray rule for a new purchase entry', async () => {
      ...
      try {
        ...
        expect(trayCalculationService.resolveTrayRule).toHaveBeenCalled();
  ...
  describe('savePurchases - Dairy Trays propagation', () => {
    it('recalculates the current Dairy Trays paper after saving purchases', async () => {
      ...
      try {
        ...
        const dairyTraysRepository = new DairyTraysRepository(prisma);
        ...
        (service as any).dependencyOrchestrator = dependencyOrchestrator;
        ...
        const dairyTrayPaper = await prisma.dairy_tray_paper.findUnique({
          where: {
            order_paper_id: orderPaper.id,
          },
        });
        ...
        const transaction = await prisma.dairy_tray_transaction.findFirst({
          where: {
  ...
  describe('savePurchases - transaction rollback', () => {
    it('rolls back purchase changes when downstream propagation fails', async () => {
      ...
      try {
        ...
        const dependencyOrchestrator = (service as any).dependencyOrchestrator;
  ...
  describe('savePurchases - concurrent saves', () => {
    it('keeps the purchase state consistent when two saves run concurrently', async () => {
      ...
      try {
         ...
         */
        expect([4, 7]).toContain(Number(entries[0].purchased_qty));
