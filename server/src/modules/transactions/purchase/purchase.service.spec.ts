...
describe('PurchaseService', () => {
    ...
    describe('getPurchases', () => {
        ...
        it('throws when the order paper does not exist', async () => {
            ...
            expect(
                purchaseRepository.findOrderPaperById,
        ...
        it('passes the transaction client to repository reads', async () => {
            ...
            expect(
                purchaseRepository.findPurchasePaper,
            ).toHaveBeenCalledWith(1, tx);

            expect(
                purchaseRepository.findPurchaseEntries,
            ).toHaveBeenCalledWith(purchasePaper.id, tx);
        });
        ...
        it('does not request live rates for allocations that already have purchase entries', async () => {
            ...
            expect(
                purchaseRepository.getProductLinksBatch,
            ).toHaveBeenCalledWith([], tx, true);
        ...
        it('returns the purchase paper updated timestamp', async () => {
            ...
            expect(result.purchasePaperUpdatedAt).toEqual(
                purchasePaper.updated_at,
            );
        });
        ...
        it('passes allocations and purchase entries through the builder pipeline', async () => {
            ...
            expect(
                purchaseBuilder.applyVehicleAllocations,
            ).toHaveBeenCalledWith(
                expect.anything(),
                [allocation],
            );
            ...
            expect(
                purchaseBuilder.applyPurchaseEntries,
            ).toHaveBeenCalledWith(
                expect.anything(),
                [purchaseEntry],
                [allocation],
            ...
            expect(
                purchaseBuilder.applyVarianceMetadata,
            ).toHaveBeenCalledWith(
                expect.anything(),
                [allocation],
                [purchaseEntry],
        ...
        it('builds the purchase workflow from the paper status', async () => {
            ...
            expect(
                workflowBuilder.buildPurchasesWorkflow,
    ...
    describe('savePurchases', () => {
        ...
        beforeEach(() => {
            ...
            purchaseValidationService.validatePurchases.mockResolvedValue(
                undefined,
            );
            ...
            purchaseRepository.replacePurchaseEntries.mockResolvedValue(
                undefined,
            );
        ...
        it('rejects stale expectedUpdatedAt', async () => {
            ...
            await expect(
                service.savePurchases(1, staleDto),
            ...
            expect(
                purchaseRepository.replacePurchaseEntries,
            ).not.toHaveBeenCalled();
        });
        ...
        it('accepts matching expectedUpdatedAt', async () => {
            ...
            await expect(
                service.savePurchases(1, matchingDto),
            ).resolves.toEqual({
                success: true,
            });
        });
        ...
        it('loads tray rules before constructing purchase rows', async () => {
            ...
            expect(
                dairyTraysRepository.getProductTrayRules,
            ).toHaveBeenCalledWith(tx);
        });
        ...
        it('preserves the existing frozen tray type', async () => {
            ...
            const rows =
                purchaseRepository.replacePurchaseEntries.mock.calls[0][1];
        ...
        it('creates a purchase paper when one does not already exist', async () => {
            ...
            expect(
                purchaseRepository.getOrCreatePurchasePaper,
            ).toHaveBeenCalledWith(1, tx);
        });
    });
});