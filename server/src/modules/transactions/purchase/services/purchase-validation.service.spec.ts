...
describe('PurchaseValidationService', () => {
  ...
  beforeEach(() => {
    ...
    service = new PurchaseValidationService(purchaseRepository);
  });

  describe('validatePurchases', () => {
    beforeEach(() => {
      purchaseRepository.findProducts.mockResolvedValue([product] as any);
    ...
    it('loads products, vehicle assignments and allocations using the provided db client', async () => {
      ...
      expect(purchaseRepository.findProducts).toHaveBeenCalledWith(tx);
    ...
    it('rejects negative purchased quantity', async () => {
      ...
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.NEGATIVE_PURCHASE_QTY);
    });
    ...
    it('rejects an invalid product', async () => {
      ...
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.INVALID_PRODUCT(999));
    });
    ...
    it('rejects an invalid vehicle', async () => {
      ...
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.INVALID_VEHICLE(999));
    });
    ...
    it('rejects when the allocation does not exist', async () => {
      ...
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.ALLOCATION_NOT_FOUND(10, 999));
    });
    ...
    it('rejects when purchased quantity exceeds allocated quantity', async () => {
      ...
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.PURCHASE_EXCEEDS_ALLOCATION);
    });
    ...
    it('rejects allocations with a null vehicle id', async () => {
      ...
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.INVALID_ALLOCATION_IDENTIFIERS);
    });
    ...
    it('validates multiple entries independently', async () => {
      ...
      await expect(
        service.validatePurchases(
          paperId,
          {
            entries: [purchaseEntry, secondEntry],
          } as any,
          tx as any,
        ),
      ).resolves.toBeUndefined();
  ...
  describe('validatePurchasesComplete', () => {
    ...
    it('loads allocations using the provided db client', async () => {
      await service.validatePurchasesComplete(paperId, tx as any);
    ...
    it('throws when there are no vehicle allocations', async () => {
      ...
      await expect(
        service.validatePurchasesComplete(paperId, tx as any),
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.NO_VEHICLE_ALLOCATIONS);
    });
    ...
    it('throws when a required allocation has no purchase entry', async () => {
      ...
      await expect(
        service.validatePurchasesComplete(paperId, tx as any),
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.PURCHASE_MISSING(10, 30));
    });
    ...
    it('accepts a missing purchase paper when all allocations have zero quantity', async () => {
      ...
      expect(purchaseRepository.findPurchaseEntries).not.toHaveBeenCalled();
    });
    ...
    it('rejects a missing purchase paper when a positive allocation is required', async () => {
      ...
      await expect(
        service.validatePurchasesComplete(paperId, tx as any),
      ).rejects.toThrow(PURCHASE_ERROR_MESSAGES.PURCHASES_NOT_COMPLETED);
  ...
  describe('validateNoDuplicateEntries', () => {
    ...
    it('rejects duplicate vehicle-distributor-category-product-session combinations', () => {
      expect(() =>
        service.validateNoDuplicateEntries([purchaseEntry, purchaseEntry]),
    ...
    it('allows an empty entry list', () => {
      expect(() => service.validateNoDuplicateEntries([])).not.toThrow();
    });
  });
});
