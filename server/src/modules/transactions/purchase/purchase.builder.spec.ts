...
describe('PurchaseBuilder', () => {
  ...
  describe('buildPurchaseGrids', () => {
    ...
    it('includes packaging type for NON_MILK products', () => {
      ...
      builder.buildPurchaseGrids(summaries as any, assignments as any);
    ...
    it('skips summaries with no products', () => {
      ...
      const result = builder.buildPurchaseGrids(summaries as any, []);

      expect(result).toEqual({
        purchases: [],
      });

      expect(productColumnsBuilder.buildGroupedColumns).not.toHaveBeenCalled();
    });
  ...
  describe('applyVehicleAllocations', () => {
    it('applies allocation quantities to matching rows', () => {
      const result = builder.applyVehicleAllocations(baseGrid, [
        allocation,
      ] as any);
    ...
    it('does not modify the original grid', () => {
      ...
      builder.applyVehicleAllocations(baseGrid, [allocation] as any);
  ...
  describe('applyPurchaseEntries', () => {
    ...
    it('initializes orphanedEntries even when there are no purchase entries', () => {
      const result = builder.applyPurchaseEntries(baseGrid, [], [
        allocation,
      ] as any);
  ...
  describe('applyPurchaseRates', () => {
    it('applies a rate to the matching row', () => {
      ...
      const result = builder.applyPurchaseRates(baseGrid, [
        {
          distributorId: 20,
          category,
          vehicleId: 10,
      ...
      expect(purchaseBillingService.calculate).toHaveBeenCalledWith(0, 25, 1);
    });
    ...
    it('uses the existing quantity when calculating the amount', () => {
      ...
      const result = builder.applyPurchaseRates(grid, [
        {
          distributorId: 20,
          category,
          vehicleId: 10,
      ...
      expect(purchaseBillingService.calculate).toHaveBeenCalledWith(10, 25, 2);
    ...
    it('ignores rates without a matching row', () => {
      ...
      expect(purchaseBillingService.calculate).not.toHaveBeenCalled();
    ...
    it('does not modify the original grid', () => {
      ...
      builder.applyPurchaseRates(baseGrid, [
        {
          distributorId: 20,
          category,
          vehicleId: 10,
  ...
  describe('applyVarianceMetadata', () => {
    it('calculates and applies variance metadata to a matching row', () => {
      ...
      expect(purchaseVarianceCalculator.calculate).toHaveBeenCalledWith(15, 12);

      expect(result.purchases[0].rows[0].product_30_variance).toEqual({
        allocatedQty: 15,
        purchasedQty: 12,
        hasVariance: true,
        variance: -3,
    ...
    it('converts allocation and purchase quantities to numbers', () => {
      ...
      expect(purchaseVarianceCalculator.calculate).toHaveBeenCalledWith(15, 15);
    });

    it('skips purchase entries without a matching allocation', () => {
      const result = builder.applyVarianceMetadata(baseGrid, [], [
        purchaseEntry,
      ] as any);

      expect(purchaseVarianceCalculator.calculate).not.toHaveBeenCalled();

      expect(result.purchases[0].rows[0].product_30_variance).toBeUndefined();
    });
