...
describe('PaperRepository', () => {
    ...
    describe('findTodayPaper', () => {
        it('should query prisma correctly', async () => {
            ...
            await repository.findTodayPaper(
                today,
                tomorrow,
            );

            expect(
                prisma.order_paper.findFirst,
    ...
    describe('findOrderPaper', () => {
        it('should query paper by date range', async () => {
            ...
            await repository.findOrderPaper(
                today,
                tomorrow,
            );
    ...
    describe('generateOrderPaper', () => {
        it('should create draft paper with next day sale date', async () => {
            ...
            await repository.generateOrderPaper(
                date,
            );

            const createCall =
                prisma.order_paper.create.mock.calls[0][0];

            expect(
                createCall.data.order_date,
            ).toEqual(date);

            expect(
                createCall.data.status,
            ).toBe('DRAFT');

            const expectedSaleDate =
                new Date(date);

            expectedSaleDate.setDate(
                expectedSaleDate.getDate() + 1,
            );

            expect(
                createCall.data.sale_date,
            ).toEqual(expectedSaleDate);
        });
    });

    describe('generateOrderSheets', () => {
        it('should create sheets using createMany', async () => {
            await repository.generateOrderSheets(
                1,
                [
                    { id: 10 },
                    { id: 20 },
    ...
    describe('submitNightEntry', () => {
        it('should update status', async () => {
            await repository.submitNightEntry(
                1,
            );

            expect(
                prisma.order_paper.update,
            ).toHaveBeenCalledWith(
    ...
    describe('submitMorningEntry', () => {
        it('should update status', async () => {
            await repository.submitMorningEntry(
                1,
            );
    ...
    describe('finalizePaper', () => {
        it('should update status', async () => {
            await repository.finalizePaper(
                1,
            );
    ...
    describe('getPaperSheets', () => {
        it('should query sheets by paper id', async () => {
            ...
            await repository.getPaperSheets(
                1,
            );

            expect(
                prisma.order_sheet.findMany,
            ).toHaveBeenCalledWith({
                where: {
                    order_paper_id: 1,
                },
    ...
    describe('getSheetItems', () => {
        it('should query sheet items with all required relations', async () => {
            ...
            await repository.getSheetItems(
                5,
            );

            expect(
                prisma.order_sheet_items.findMany,
            ).toHaveBeenCalledWith({
                where: {
                    order_sheet_id: 5,
                },
