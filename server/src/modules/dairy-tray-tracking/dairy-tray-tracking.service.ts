...
export class DairyTrayTrackingService {
  ...
  async getDairyTrayGrid(paperId: number) {
    ...
    if (!this.workflowStateService.canEditDairyTrayTracking(paper.status)) {
    ...
    if (previousPaper) {
      ...
      if (previousDairyTrayPaper) {
        previousTransactions = await this.repository.getPreviousTrayBalances(
  ...
  async saveDairyTrayEntries(paperId: number, dto: SaveDairyTrayEntriesDto) {
    ...
    this.validation.validateSaveRequest(dto.entries, vehicles, trayTypes);
