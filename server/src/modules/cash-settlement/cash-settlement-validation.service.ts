...
export class CashSettlementValidationService {
  ...
  private validateRouteDenominationsFromPaper(
    ...
    for (const sheet of paper.order_sheet) {
      ...
      const denominationTotal =
        Number(sheet.cash_route_settlement?.note_2000 ?? 0) * 2000 +
        Number(sheet.cash_route_settlement?.note_500 ?? 0) * 500 +
        Number(sheet.cash_route_settlement?.note_200 ?? 0) * 200 +
        Number(sheet.cash_route_settlement?.note_100 ?? 0) * 100 +
        Number(sheet.cash_route_settlement?.note_50 ?? 0) * 50 +
        Number(sheet.cash_route_settlement?.note_20 ?? 0) * 20 +
        Number(sheet.cash_route_settlement?.note_10 ?? 0) * 10 +
        Number(sheet.cash_route_settlement?.coins ?? 0);
  ...
  private validateBankDepositsFromPaper(
    ...
    const totalRouteNetCash = paper.order_sheet.reduce((total, sheet) => {
      const routeCash = sheet.client_collection.reduce(
        (sum, collection) =>
          sum +
          Number(collection.office_amount_given) +
      ...
      return total + routeCash - expenseTotal;
    }, 0);

    const directCollectionTotals = this.getDenominationTotals(
      paper.cash_direct_collections,
    );

    const depositTotals = this.getDenominationTotals(paper.cash_bank_deposits);

    const totalDirectCollections = this.getDenominationAmount(
      directCollectionTotals,
    );

    const totalDeposits = this.getDenominationAmount(depositTotals);

    const officeCash = totalRouteNetCash + totalDirectCollections;
    ...
    const routeDenominationTotals = this.getDenominationTotals(
      routeDenominationRows,
    );

    const availableDenominations = {
      note2000:
        routeDenominationTotals.note2000 + directCollectionTotals.note2000,

      note500: routeDenominationTotals.note500 + directCollectionTotals.note500,

      note200: routeDenominationTotals.note200 + directCollectionTotals.note200,

      note100: routeDenominationTotals.note100 + directCollectionTotals.note100,

      note50: routeDenominationTotals.note50 + directCollectionTotals.note50,

      note20: routeDenominationTotals.note20 + directCollectionTotals.note20,

      note10: routeDenominationTotals.note10 + directCollectionTotals.note10,

      coins: routeDenominationTotals.coins + directCollectionTotals.coins,
    };
  ...
  private getDenominationTotals(rows: DenominationRow[]): DenominationTotals {
  ...
  private getDenominationAmount(totals: DenominationTotals): number {
    return (
