...
export class CashSettlementRepository {
  ...
  async replaceRouteExpenses(sheetId: number, expenses: RouteExpenseDto[]) {
  ...
  async saveRouteDenomination(denomination: RouteDenominationDto) {
  ...
  async replaceBankDeposits(paperId: number, deposits: BankDepositDto[]) {
    await this.prisma.$transaction(async (tx) => {
